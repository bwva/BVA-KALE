...
# 		_line_skip		=> $args{line_skip}						|| '^($| *\#| *\<\!)',
		...
		_tracking		=> $args{tracking}						|| [ @BVA::KALE::DATA::PROC::tracks ],
		_sort_packs		=> { %BVA::KALE::DATA::META::sort_packs,	%{ $args{sort_packs}	|| {} } },
		_record_packs	=> { %BVA::KALE::DATA::META::record_packs,	%{ $args{record_packs}	|| {} } },
		_fld_packs		=> { %BVA::KALE::DATA::META::fld_packs,		%{ $args{fld_packs}		|| {} } },
		_fld_fmts		=> { %BVA::KALE::DATA::META::fld_fmts,		%{ $args{fld_fmts}		|| {} } },
		_nulls			=> { %BVA::KALE::DATA::META::nulls,			%{ $args{nulls}			|| {} } },
	);
...
BVA::KALE::DATA->init_from_dbx(\\%_DBX);
...
# 	or die "Database statement execution failure: " . $dbh->messages('err');
	...
	$prep_sttmt->{_procs} 		= [ map {
			...
			if ($proc) {
				...
				} elsif (defined &{ "BVA::KALE::DATA::PROC::$proc" }) {
					$process						= \&{ "BVA::KALE::DATA::PROC::$proc" };
				}
				if ($process) {
					$args =~ s/^\s*(.*?)\s*[,\)]*$/$1 || ''/e;
...
sub prep_index {
	...
	if ($statement{WITH}->{idx_upcase}) {
		...
		push @{ $statement{_procs} } => \&BVA::KALE::DATA::PROC::upcase;
	} elsif ($statement{WITH}->{idx_lowcase}) {
		push @{ $statement{_procs} } => \&BVA::KALE::DATA::PROC::lowcase;
...
sub prep_indexid {
	...
	$statement{INDEX}		= {
		...
		id_col	=> $self->id_col,
		msg		=> '',
		idxpat	=> join(' ' => $self->pack_patterns($self->id_fld), "Z*", "Z*"),
	};
...
sub prep_create {
	...
	for my $i (0..$#flds) {
		...
		if (defined $statement{WITH}->{id_col} and $i == $statement{WITH}->{id_col}) {
			$flds[$i] = "#$flds[$i]";
		} elsif ( $statement{WITH}->{id_fld} and $flds[$i] eq $statement{WITH}->{id_fld} ) {
			$flds[$i] = "#$flds[$i]";
		}
	}
...
sub do_update {
	...
	LINE: while ($self->{CURRENT}{LINE} = <>) {
		...
		CALC: for my $col (0..$#flds) {
			...
			} elsif (defined( &{"BVA::KALE::DATA::CALC::$calc"} ) ) {
				$calc_sub	=  $self->{_calcs}->{$calc}	= \&{"BVA::KALE::DATA::CALC::$calc"};
			} else {
 				# $calc_sub	=  \&BVA::KALE::DATA::CALC::_default;
				next CALC;
			}
...
sub do_import {
	...
	FROM: for my $src (@{ $self->{_from} }) {
 		my $import_dbh		= BVA::KALE::DATA->init(input_sep => $input_sep, record_sep => $record_sep)->connect(db=>$src)
								or next FROM;
...
sub do_browse {
	...
	BROWSE: for my $src (@{ $self->{_from} }) {
		...
		LINE: while ($self->{CURRENT}{LINE} = <IN>) {
			...
			if ($self->{CURRENT}{LINE} =~ /$self->{_line_skip}/) {
				$self->{CURRENT}{LINE_START_BYTE}	= tell IN;
				next LINE;
			}
			...
			unless ( $self->$selector($self->{CURRENT}{LINE})  ){
				$self->{CURRENT}{LINE_START_BYTE}	= tell IN;
				next LINE;
			}
...
# 				}
		...
		$self->{CURRENT}{LINE_START_BYTE}	= $self->{MARK};
...
sub do_indexid {
	...
	INDEX: for my $src (@{ $self->{_from} }) {
		...
		$self->{CURRENT}{_table_idx}	= ($self->{CURRENT}{FILE} =~ /^(.*?)\.(\w{1,4})$/)[0] . '_index' . $self->{_idx_suf};
...
# 				$self->{TO}->[3]	= $self->db_struct(@{ $self->iterator('hdr') }) . "$self->{_record_sep}$self->{_record_sep}"
		...
		if ($self->{TO}->[0] eq 'dbh') {
			return BVA::KALE::DATA->init(input_sep => "\t")->connect(db => $self->{TO}->[1]);
		} elsif ($self->{TO}->[0] eq 'dbx') {
			return BVA::KALE::DATA->init(input_sep => "\t")->connect(db => $self->{TO}->[1])->save_connection;
		} else {
			return $self->{TO}->[1];
		}
...
sub get_indexes {
	...
	for my $idx (@current_indexes) {
		my ($idx_name,$idx_case,$idx_flds,$idx_age);
		$idx_age	= -M $idx;
		if ($idx =~ /^.*_index$self->{_idx_suf}$/) {
			$idx_flds	= [ $self->{_id_fld} ];
			$idx_case	= 'ncase';
...
sub msgAdd {
	my $self	= shift;
	my $new		= shift || '';
	return $self->{_msg}->("add: $new");
}

sub msgLog {
	my $self	= shift;
	my $new		= shift || '';
	return $self->{_msg}->("log: $new");
}

sub msgSay {
	my $self	= shift;
	my $new		= shift || '';
	return $self->{_msg}->("print: $new");
}

sub msgMute {
	my $self	= shift;
	my $new		= shift || '';
	return $self->{_msg}->("mute: $new");
}

sub msgUnmute {
	my $self	= shift;
	my $new		= shift || '';
	return $self->{_msg}->("unmute: $new");
}
