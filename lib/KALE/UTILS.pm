package BVA::KALE::UTILS;

$BVA::KALE::UTILS::VERSION			= '1.02_001';   # 2013-03-31
...
sub dollars {
	my $self			= shift;
	my $amt				= shift || 0;
	$amt				= sprintf qq{%.02f}, $amt;
	my ($sign,$base,$decimal)	= ('',0,0);
	if ($amt =~ /^([+-])?([0-9]+)(?:\.(\d+))$/) {
		($sign,$base,$decimal)	= ($1,$2,$3);
	} else {
		($base,$decimal)= ($amt,0);
	}
	1 while $base =~ s/^(\d+)(\d{3})/$1,$2/;
	$sign				||= '';
	return sprintf qq{%s\$%s.%.02d}, $sign,$base,$decimal;
}
...
sub ordinalize {
	...
	my $number	= shift;
	my $suffix	= '';
	return $number unless $number and $number =~ /\d+$/;
	if ($number =~ /(?<!1)1$/) {
		   $suffix = 'st';
	} elsif ($number =~ /(?<!1)2$/) {
		   $suffix = 'nd';
...
sub string_between {
	...
	@items			= sort @items;
	return (
		$test_item gt $items[0]
			and
		$test_item lt $items[-1]
	);
}
...
sub string_within {
	...
	@items			= sort @items;
	return (
		$test_item ge $items[0]
			and
		$test_item le $items[-1]
	);
}
...
If the first arg is a string, it must start with a non-negative integer number, which serves as
the quantifier for the number of elements returned.

The string's number may be followed by a colon followed by an
expression of zero length or longer; the expression will be used as
the default value for all elements. The expression may be white space.
With a plain number arg, '' will be used for all missing and undefined elements.

If the first arg is an array ref, the number of the array's elements is the group quantifier,
...
sub HTML_de_entify {
	...
	return unless defined($orig);
	study $orig;
    # thanks to Lincoln Stein and Randal Schwartz for the correct solution to this one
    $orig =~ s{&(.*?);}
    			{
				local $_ = $1;
				/^amp$/i	? "&" :
...
sub urlify {
	...
	$text =~ s{((\s*<?)(mailto:)?\b([${ltrs}${punc}]+@[${any}]+?)\s*(?!</a>)(?=[$punc]*[^${any}]|$))}
				{qq{$2<a href="mailto:$4">$4</a>} }egoi;
	# Convert URLs
	$text =~ s{(([$ante]?)(\s*<?)\b(${urls}:[$any]+?)\s*(?!</a>)(>|\&gt;)?(?=[$punc]*[^$any]|$))}
...
#  		$line			=~ s{(\S[^>]) *\n *([^<]\S+)}
	...
	for (keys %{ $urls }) {
		$formatted_msg			=~ s{(<|\&lt;)?\[$_\](>|\&gt;)?}{ $self->urlify($urls->{$_}) }e;
	}
...
#	my @web_hex_keys	= grep { /#[a-f0-9]{2}[a-f0-9]{2}[a-f0-9]{2}/ } keys %BVA::KALE::UTILS::HEXES_PLUS;
	my @web_hex_keys	= grep { /#([a-f0-9])\1([a-f0-9])\2([a-f0-9])\3/ } keys %BVA::KALE::UTILS::HEXES_PLUS;

	[ @BVA::KALE::UTILS::HEXES_PLUS{@web_hex_keys} ]
}

sub color_to_hex {
	my $self	= shift;
	my @colors	= ref($_[0]) =~ 'ARRAY' ? @{ $_[0] } :
		@_ ? @_ : sort keys %BVA::KALE::UTILS::COLORS;

	my @color_errors;

	my @hexes;
	COLOR: for (@colors) {
		if (exists $BVA::KALE::UTILS::COLORS{$_}) {
			push @hexes => $BVA::KALE::UTILS::COLORS{$_};
			next COLOR;
		}
		...
		if (exists $BVA::KALE::UTILS::COLORS{$color_try}) {
			push @hexes => $BVA::KALE::UTILS::COLORS{$color_try};
			next COLOR;
		}
	...
	wantarray ? @hexes : @color_errors ?
		'Color names not recognized: ' . join(' ' => @color_errors) : 0;

}

sub hex_to_color {
	my $self	= shift;
	my @hexes	= ref($_[0]) =~ 'ARRAY' ? @{ $_[0] } :
		@_ ? @_ : sort keys %BVA::KALE::UTILS::HEXES;
	...
	HEX: for (@hexes) {
		$_	= lc($_);
		if (exists $BVA::KALE::UTILS::HEXES{"$_"}) {
			push @colors => $BVA::KALE::UTILS::HEXES{"$_"};
			next HEX;
		}
	...
	wantarray ? @colors : @hex_errors ?
		'Hex names not recognized: ' . join(' ' => @hex_errors) : 0;
...
sub color_to_hex_plus {
	my $self	= shift;
	my @colors	= ref($_[0]) =~ 'ARRAY' ? @{ $_[0] } :
		@_ ? @_ : sort keys %BVA::KALE::UTILS::COLORS_PLUS;

	my @color_errors;

	my @hexes;
	COLOR: for my $clr (@colors) {
		if (exists $BVA::KALE::UTILS::COLORS_PLUS{$clr}) {
			push @hexes => $BVA::KALE::UTILS::COLORS_PLUS{$clr};
			next COLOR;
		}
		...
		if (exists $BVA::KALE::UTILS::COLORS_PLUS{$color_try}) {
			push @hexes => $BVA::KALE::UTILS::COLORS_PLUS{$color_try};
			next COLOR;
		}
		...
		if ($clr =~ /^\#?([0-9a-f]{3}([0-9a-f]{3})?)$/i  ) {
			...
			if (exists $BVA::KALE::UTILS::COLORS_PLUS{$colorFromHex}) {
				push @hexes => $BVA::KALE::UTILS::COLORS_PLUS{$colorFromHex};
			} else {
				push @hexes => "#$hex";
			}
			next COLOR;
...
sub hex_to_color_plus {
	my $self	= shift;
	my @hexes	= ref($_[0]) =~ 'ARRAY' ? @{ $_[0] } :
		@_ ? @_ : sort keys %BVA::KALE::UTILS::HEXES_PLUS;
	...
	HEX: for (@hexes) {
		$_ = lc($_);
		if (exists $BVA::KALE::UTILS::HEXES_PLUS{"$_"}) {
			push @colors => $BVA::KALE::UTILS::HEXES_PLUS{"$_"};
			next HEX;
		}
...
sub hex_to_color_w_hex_plus {
	...
	HEX: for (@hexes) {
		my $hex	= lc($_);
		if (exists $BVA::KALE::UTILS::HEXES_PLUS{$hex}) {
 			my $color	= $BVA::KALE::UTILS::HEXES_PLUS{$hex} =~ /$hex/i ? $hex : "$BVA::KALE::UTILS::HEXES_PLUS{$hex} ($hex)";
			push @colors => $color;
			next HEX;
		}
...
%BVA::KALE::UTILS::HEXES	= reverse %BVA::KALE::UTILS::COLORS;


%BVA::KALE::UTILS::HEXES_PLUS	= (
	'#000000'	=> 'Black',
	'#000033'	=> '#000033',
	'#000066'	=> '#000066',
	'#000080'	=> 'Navy',
...
sub mime_to_suffix {
	my $self	= shift;
	my $mime	= shift;
	$BVA::KALE::UTILS::MIMES{$mime} || $mime;
}
