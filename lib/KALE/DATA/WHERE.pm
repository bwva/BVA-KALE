package BVA::KALE::DATA::WHERE;

$BVA::KALE::DATA::WHERE::VERSION	= '1.030_001';	# 2013-03-31 bva@cruzio.com
...
# use BVA::KALE::DATA::WHERE::Parser;
...
sub _chunker {
	...
	for my $part (@parts) {
		...
		if (ref($part) eq 'ARRAY') {
			...
			push @chunks => $self->_chunker($1), 'OR';		# **
...
sub _parse_conditions {
	...
	my @conditions;
 	CONDITION: for my $condition_expr (@{ $item }) {
		...
		if (!$fld) {
			my %cur_cond		= $self->_sttmts_to_tests("", $rel, $val);
			$cur_cond{error}	= 1;
			my @subconditions	= ('S');
			...
			for my $cfld (@cond_flds) {
 			...
 			push @conditions => $parse_depth > 0 ? $self->_assemble_test_subs(@subconditions) : $self->_final_selector(@subconditions);
		} elsif ($fld eq '?' or $fld eq '_any') {
			my %cur_cond		= $self->_sttmts_to_tests($fld, $rel, $val);
			$cur_cond{depth}	= $parse_depth;
			my @subconditions	= ('S');
			push @subconditions => \%cur_cond;
			push @conditions => $parse_depth > 0 ? $self->_assemble_test_subs(@subconditions) : $self->_final_selector(@subconditions);
		} elsif ($self->column($fld)) {
			my %cur_cond		= $self->_sttmts_to_tests($fld, $rel, $val);
			$cur_cond{depth}	= $parse_depth;
			my @subconditions	= ('S');
			push @subconditions => \%cur_cond;
			...
			my %cur_cond		= $self->_sttmts_to_tests(qq{Unknown Field "$fld"}, $rel, $val);
			$cur_cond{depth}	= $parse_depth;
			$cur_cond{error}	= 1;
			my @subconditions	= ('S');
			push @subconditions => \%cur_cond;
...
sub _sttmts_to_tests {
	my $self	= shift;
	my ($fld, $rel, $val)	= map { defined && length($_) ? $_ : '' } @_;
#	$fld		= $fld eq '?' ? '_any' : $fld;
	...
	$current_condition{phrase}	= $op->{phrase} || 'Huh?';
...
sub _assemble_test_subs {
	...
	if ($test{type} eq 'S') {
		...
		$test{depth}	= join '-' => map { $has_error += $_->{error} || 0; $_->{depth} || 0 } @tests;
		...
		$test{depth}	= join '-' => map {$has_error += $_->{error} || 0; $_->{depth} || '' } @tests;
