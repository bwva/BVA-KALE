...
# our ($KEY, @KEY, %KEY); # KEY
# our ($KALE, @KALE, %KALE);
# our ($OUTPUT, @OUTPUT, %OUTPUT);
# our ($INPUT, @INPUT, %INPUT);

use parent qw{ BVA::KALE::IN BVA::KALE::DB BVA::KALE::DATA BVA::KALE::OUT BVA::KALE::UTILS BVA::KALE::DATETIME };
...
sub init {
	my ($class, @args)	= @_;
	{
		...
		/HASH/i		and	*KEY						= $args[0]
	 	  or
		/ARRAY/		and	@KEY{qw/_env _start _end/}	= @{ $args[0] }
		  or
						@KEY{qw/_env _start _end/}	= @args;
	}
	...
	$KEY{_start}			||= $KEY{_env} =~ /^(htm|psgi)/i	? '<!--'	: '#';
	$KEY{_end}				||= $KEY{_env} =~ /^(htm|psgi)/i	? '-->'		: '#';
...
# 	my $ui_obj				=  bless \*{$KEY{_ui_mark}}, $class;
#  	$KEY{_generator}		=  $ui_obj;

# 	{
# 		no strict 'refs';
# 		*{"$KEY{_ui_mark}"}		= *KEY;
#  		push @{ *KEY } 			=> \*{"$KEY{_ui_mark}"};
# 	}
...
sub new {
	...
	{
		local $_	= ref($args[0]);
		/HASH/i		and %properties										= (%properties, %{ $args[0] })
	 	 or
		/ARRAY/		and @properties{qw/_mark _file _start _end _sub/}	= @{ $args[0] }
		  or
					@properties{qw/_mark _file _start _end _sub/}		= @args;
	}
	...
	$properties{_obj_init_time}	= BVA::KALE::DATETIME::tell_time('secs',time);
	...
	## Default template
	$properties{_default_tmpl}	= $properties{'_is_direct'} ?
	...
	if ($properties{_mark}) {
		if ($properties{_file} !~ /[\^-]/) {
			no strict 'refs';
			open (\*{"main::" . $properties{_mark}}, "+>>", $properties{_file} ) 
				or carp "Can't open file $properties{_file}: $!";
			*{"main::$properties{_mark}"}{IO}->autoflush();
			$properties{_file_as_opened}	= [ 'File', *{"main::$properties{_mark}"}{IO}->fileno(), *{"main::$properties{_mark}"}{IO}->stat() ];
			$properties{_file_keep_open}	= 1;
			#chmod oct(777), $properties{_file};
		}
		no strict 'refs';
...
sub objects ($;@) {
	my ($self, @object_names)	= @_;
   	local *KEY	= invert($self);
# 	my $ui	= $KEY{_generator};
#   	local *KALE	= invert($ui);

   	my @objs	= grep { ref($_) eq __PACKAGE__ } @{ $KEY{_objects} };
#  	my @objs	= grep { ref($_) eq __PACKAGE__ } @{ $KEY->{_generator}->data('_objects') };
	...
	my %objs	= map {  ${ *$_ }{_mark} => $_ } @objs;
  	...
  	return map { $objs{$_} ? $objs{$_} : () } @object_names;
}
...
sub direct_old ($;@) {
	my ($self)	= @_;	
 	...
 	unless (exists $KEY{_direct} ) {
		...
		my $match_str1	= qr{(?sx: ((\Q${start}${mark}\E)	(?:\:|\s|\b)	((.(?!\2))+?)	\Q${end}\E) )};
		my $match_str2	= qr{(?sx: ((\Q${owner}\E[:.])						((\S(?!\2))+?)	(?:\b))   	)};
...
#  	local *KEY	= invert($self);
#  	
# 	return $KEY{_direct} if $KEY{_is_direct};
# 	
# 	my $owner		= $KEY{_mark};
# 	my $owner_start	= $KEY{_start};
# 	my $owner_end	= $KEY{_end};
# 	my $mark		= qr/[:.~]/; # $KEY{_mark}; #
# 	my $start		= $KEY{_direct_start}	|| '[';
# 	my $end			= $KEY{_direct_end}		|| ']';
# 	my $match_str1	= qr{(?sx: ((\Q${start}\E${mark})	(?:\:|\s|\b)	((.(?!\2))+?)	\Q${end}\E) )};
# 	my $match_str2	= qr{(?sx: ((\Q${owner}\E${mark})						((\S(?!\2))+?)	(?:\b))   	)};
# # 	my $match_str1	= qr{(?sx: ((\Q${start}\E[:.~])	(?:\:|\s|\b)	((.(?!\2))+?)	\Q${end}\E) )};
# # 	my $match_str2	= qr{(?sx: ((\Q${owner}\E[:.~])					((\S(?!\2))+?)	(?:\b))   	)};
# 	$KEY{_direct}	= $KEY{_generator}->new({
# 		_is_direct		=> 1,
# 		_owner_mark		=> $owner,
# 		_owner_start	=> $owner_start,
# 		_owner_end		=> $owner_end,
# 		_direct_start	=> $start,
# 		_direct_end		=> $end,
# 		_mark			=> $mark,
# 		_start			=> $start,
...
# 		},
# 	});
# 	# Remove the parent object's data (but not its meta-data)
# 	$KEY{_direct}->clear();
# 	
# 	return $KEY{_direct};
# }
...
## uses the UI object's _direct_mark, _direct_start, and _direct_end if the UI object has them.
...
# # 		local *KALE	= \*{$obj};
# 		$field 		=~ s/^\s*(.*?)\s*$/$1/s;
# 		my $out		= defined $KALE{$field} ? $KALE{$field} : "";
...
# 	my $match_str1		= qr{(?sx: ((\Q${start}${mark}\E)	(?:\:|\s|\b)	((.(?!\2))+?)	\Q${end}\E) )};
# 	my $match_str2		= qr{(?sx: ((\Q${owner}\E[:.])						((\S(?!\2))+?)	(?:\b))   	)};
# 	$KALE{_match_str}	= qr{(?sx: $match_str1 | $match_str2 )};
# 	$KALE{_is_direct}	= 1;
# 	$KALE{_owner}		= $owner;
# 
#  	return _process_tmpl( \*KALE , $KALE);
# }
...
sub resolve ($@) {
	...
	local *KALE	= sub {
		...
		$field 		=~ s/^\s*(.*?)\s*$/$1/s;
# 		my $out		= defined $KALE{$field} ? $KALE{$field} : "";
		my $out		= defined $KALE{$field} ? $KALE{$field} : "[:$field]";
#		$KALE{__DONE__}++ unless $out;
		return $out;
	};
...
sub process {
	...
	my @products	= map { KEY(\*KEY, $_) || OUTPUT(\*KEY, $_) || $_ } @strings;
...
sub attach_file {
	my ($self, $file,$keep_open)	= @_;
  	local *KEY			= invert($self);
  	$file				||= undef;
  	$keep_open			||= 0; # only affects named files, not tmp files or scalar variable

	if ( ! $file ) {
		if (open KEY, "+>>", undef) {
			*KEY{IO}->autoflush();
			$KEY{_file_as_opened}	= [ 'Tmp', *KEY{IO}->fileno(), *KEY{IO}->stat() ];
		...
		$KEY{_vfile}	= '';
		if (open KEY, "+>>", \$KEY{_vfile}) {
			*KEY{IO}->autoflush();
			$KEY{_file}		= $file;
			$KEY{_file_as_opened}	= [ 'Virtual', *KEY{IO}->fileno(), *KEY{IO}->stat() ];
			$KEY{_file_keep_open}	= 1;
		} else {
			warnings::warnif( 'io', "Problem with attaching temp file:\n$!");
			charge_err(\*KEY, "Problem with attaching temp file:\n$!") and return;
		...
		if (open KEY, "+>>", $file) {
			*KEY{IO}->autoflush();
			$KEY{_file}	= $file;
			$KEY{_file_as_opened}	= [ 'File', *KEY{IO}->fileno(), *KEY{IO}->stat() ];
		...
		carp "Proposed attached file is neither a filehandle nor a valid filename.";
	}
	\*KEY
}
...
sub flush_refsOK ($) {
	...
	foreach my $elem (@KEY) {
 	...
 	@KEY			= @globs;
	return wantarray ? @buf : @refs ? [ @refs, join '' => @buf ] : join '' => @buf;
}
...
sub flush_random ($;$) {
	...
	foreach my $elem (@KEY) {
 	...
 	@KEY			= @globs;
 	$num_elems		= scalar @buf;
 	$size			= $size ? $size > $num_elems ? $num_elems : $size : $num_elems;
	...
	for (1..$size) {
		...
		redo if $random{$rand_num}++;
		push @random_series => $rand_num;
	}
	
	return @buf[@random_series];
}
...
## Template: Returns a template; the first optional arg
...
sub template ($;@) {
	...
	my $tmpl		= '';
	my $tmpl_name = shift || '';
	my ($get_tmpl_file,$template_name);

	($get_tmpl_file,$template_name) = $tmpl_name =~ /^(-)?([a-zA-Z_][\w -]*?)\s*$/;
...
sub _save_template_to_file {
	...
	if ($template_name) {
		open my $t, ">", "$KEY{_display_dir}/$template_name" . ".tmpl"
			or return "";
		print $t $tmpl, "\n";
		close $t;
	}
...
sub save_sys_list ($;@) {
	...
	unless (-d $list_dir) {
		mkdir($list_dir, oct(777)) or carp "List dir creation failed for $list_dir: $!";
	}
...
## Data read back is JSON::decoded into a hash assumping that it is valid json .
## The hash is then added to the existing %KEY, over-writing fields with
## the same names. Allows multiple serialized hashes to be read in, in the order
## they were stored, resulting in the latest values for fields stored more than once.
## Does not process data; does not alter the file.
...
sub retrieve_data {
	...
	unless (defined *KEY{IO} and *KEY{IO}->opened() ) {
		if ( ! $KEY{_file} ) {
			...
			$KEY{_vfile} ||= '';
			open KEY, "+>>", \$KEY{_vfile} or return qq{};
		} else {
			open KEY, "+>>", "$KEY{_file}" or return qq{};
		}
	}
	seek(KEY, 0, 0);
...
## a newline. This updates the value in the target field.
...
sub get_displays {
	...
	my $displays	= BVA::KALE::DATA->init->connect(file => "$display_dir/$display_db")
...
sub add_display {
	...
	my $all_displays_query					= $displays->prepare(
		qq{ SELECT name FROM me WHERE all TO cursor: hash }
	);

	$all_displays_query->execute();
	while ( my $r = $all_displays_query->fetchrow_hashref() ) {
		if ($r->{name} eq $new_display) {
			oops(qq{Display \"$new_display\" already exists.<br>\nPlease choose a different name.}) 
		}
	}
...
sub reveal ($;@) {
	my $self				= shift();
	local *KEY				= $self->invert();  #ref($self) eq __PACKAGE__ ? $self : *{ $self };
	...
	RENDER: {
		...
		foreach my $obj ( @objects ) {
			...
			push @renderlines => qq{<pre>\nProcess:\nMark\tStart\tEnd\n}, BVA::KALE::UTILS::HTML_entify(qq{$mark\t$start\t$end}), qq{\n</pre>\n};
